...
function updateHeaderState() {
    ...
    const syncActiveState = (links: NodeListOf<HTMLAnchorElement>) => {
        ...
        // Base path is the first link in navigation (Home root, e.g. /journal or /)
        const basePath = links.length > 0 ? normalize(links[0].pathname) : '/';

        links.forEach((link) => {
            const linkPath = normalize(link.pathname);
            const isActive = linkPath === basePath
